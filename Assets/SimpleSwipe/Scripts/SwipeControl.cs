...
public class SwipeControl : MonoBehaviour {
    ...
    public SWIPE_TYPE m_stSwipeType = SWIPE_TYPE.ST_4_DIRECTION;
    public bool m_bool_timer_Mouse = false;

	[Range(0.0f, 10.0f)]
	public  float SwipeDistance 					= 0.2f;

    public float Timer_Mouse = 1f;
	...
	private void UpdateMouseBasedControl() {
		...
		if ( !m_bSwipePressed && Input.GetMouseButtonDown( m_iInputMouse ) ) {
            ...
            m_bool_timer_Mouse = true;
		}
        
		if ( m_bSwipePressed ) {

            if (Input.GetMouseButtonUp(m_iInputMouse))
            {
                m_bSwipePressed = false;
                m_vEndSwipePos = Input.mousePosition;

                if (m_bool_timer_Mouse = true)
                {
	...
	private void Update() {
        ...
        if (m_bool_timer_Mouse == true)
        {
            Timer_Mouse -= Time.deltaTime;
        }
        if (Timer_Mouse < 0)
        {
            m_bool_timer_Mouse = false;
        }
	}
}
